...
// let device_token = 'c7k8jdHIT4q886YQgETj-K:APA91bFgNj7p_3ZlfPoNfm1DwMW_f89jY-qPa754guW78AzMGoEMPVIadiUPJCsiDyzmtFGye-o3DMT30hgV6xD9Zg3g_d-Wo-3GBFn6Ht5FU00UsVrcu4CWq66bddw0_rhe__fCbjpp';
// common.sendNotification(device_type,device_token);
...
let placeOrder = (data, headers, cb) => {
    ...
    async.auto({
        checkUserExistsinDB: (cb) => {
            ...
            if (!data.order_unique_id || !data.address_id || !data.address_type || !data.vendor_id || !data.order_price || !data.payment_mode || !data.order_type || !data.address || !data.lat || !data.lng) {
                cb(null, { "statusCode": util.statusCode.BAD_REQUEST, "statusMessage": util.statusMessage.PARAMS_MISSING })
                return;
            }
            var userId
            ...
            let dataToSet = {
                ...
                user_id: userId,
                currency: 'Rs',
                delivery_charge: data.shipping_charge ? data.shipping_charge : 0,
                sub_tab: data.sub_tab ? data.sub_tab : '',
                address_type: data.address_type ? data.address_type : '',
                discount: data.discount ? data.discount : '',
                coupon_code: data.coupon_code ? data.coupon_code : '',
                total_item_price: data.total_item_price ? data.total_item_price : 0,
                            ...
                            customerDAO.getUserData(criteria, (err1, dbData) => {
                                ...
                                if (err1) {
                                    ...
                                    let order_data = {
                                        ...
                                        'lat': data.lat,
                                        'lng': data.lng,
                                        'address_name': data.address_user?data.address_user:'',
                                        'address': data.address
                                    }
                                    ...
                                    customerDAO.getUserData(criteria2, (errv, dbDatavendor) => {
                                        console.log(errv)
                                        if (errv) {
                                            cb(null, { "statusCode": util.statusCode.INTERNAL_SERVER_ERROR, "statusMessage": util.statusMessage.DB_ERROR })
                                            return;
                                        } else {
                                         if(dbDatavendor && dbDatavendor.length){
                                           ...
                                           let insertData2 = {
                                            ...
                                            type: 'order',
                                            order_id: insertData['0'].order_id
                                          }
                                          customerDAO.addNotification(insertData2, (erran, insertnoti) => {
                                            console.log(erran)
                                            if (erran) {
                                                cb(null, { "statusCode": util.statusCode.INTERNAL_SERVER_ERROR, "statusMessage": util.statusMessage.DB_ERROR })
                                            } else {
                                        if(dbDatavendor[0].device_token !='' && dbDatavendor[0].device_token != '0'){
                                        
                                            customerDAO.getNotificationCount(criteria2, (errn, noticount) => {
                                                if (errn) {
                                                    console.log(errn)
                                                    cb(null, { "statusCode": util.statusCode.INTERNAL_SERVER_ERROR, "statusMessage": util.statusMessage.DB_ERROR })
                                                } else {   
                                        let badge = noticount[0].count?noticount[0].count+1:1;
                                        common.sendNotification(dbDatavendor[0].device_type, dbDatavendor[0].device_token,title, message, notitype, badge,insertData['0'].order_id);  
                                                }
                                            })
...
let orderList = (data, headers, cb) => {
    async.auto({
        checkUserExistsinDB: (cb) => {
            ...
            let criteria = {
                user_id: userId,
                order_status: data.order_status ? data.order_status : "pending",
                order_type: data.order_type ? data.order_type : "normal",
                tab: data.tab ? data.tab : ""
            }
            customerDAO.getUserData(criteria, (err, dbData) => {
                if (err) {
                ...
                if (dbData && dbData.length == 0) {
                    cb(null, { "statusCode": util.statusCode.BAD_REQUEST, "statusMessage": util.statusMessage.INCORRECT_USER });
                    return;
                } else {
                    customerDAO.getOrders(criteria, (err, dbData1) => {
                        if (err) {
                            cb(null, { "statusCode": util.statusCode.FOUR_ZERO_FOUR, "statusMessage": util.statusMessage.DB_ERROR });
                            return;
                        } else {
                            cb(null, { "statusCode": util.statusCode.OK, "statusMessage": "Order Listing", "result": dbData1 });
                            return;
                        }
                    });
                }
...
//=================== order detail ======================//
let getOrderDetails = (data, headers, cb) => {
    async.auto({
            checkUserExistsinDB: (cb) => {
                if (!data.order_id) {
                    cb(null, { "statusCode": util.statusCode.BAD_REQUEST, "statusMessage": util.statusMessage.PARAMS_MISSING })
                ...
                customerDAO.getUserData(criteria, (err, dbData) => {
                    ...
                    if (dbData && dbData.length == 0) {
                        ...
                        customerDAO.get_order_data(criteria1, (err1, dbData1) => {
                            if (err1) {
                                cb(null, { "statusCode": util.statusCode.FOUR_ZERO_FOUR, "statusMessage": util.statusMessage.DB_ERROR });
                                return;
                            } else {
                                customerDAO.get_order_details(criteria1, (err2, dbData2) => {
                                    if (err2) {
                                        cb(null, { "statusCode": util.statusCode.FOUR_ZERO_FOUR, "statusMessage": util.statusMessage.DB_ERROR });
                                        return;
                                    } else {
                                        
                                           if (dbData1['0'].address_id != '0') {
                                                let addresscriteria = {
                                                    "address_id": dbData1['0'].address_id
                                                }
                                                customerDAO.getAddressData(addresscriteria, (err3, userAddress) => {
                                                        if (err3) {
                                                            cb(null, { "statusCode": util.statusCode.FOUR_ZERO_FOUR, "statusMessage": util.statusMessage.DB_ERROR });
                                                            return;
                                                        } else {
                                                            cb(null, { "statusCode": util.statusCode.OK, "statusMessage": "Order Data", "order_data": dbData1['0'], "order_details": dbData2, "user_address": userAddress });
                                                            return;
                                            ...
                                            //     else {
                                            //         let userAddress = [];
                                            //         cb(null, { "statusCode": util.statusCode.OK, "statusMessage": "Order Data", "order_data": dbData1['0'], "order_details": dbData2, "restaurant": restaurantData, "user_address": userAddress });
                                            //         return;
                                            // }
                                        }
                                   });
...
//=============== add to wallet =============//
let addWalletAmount = (data, headers, cb) => {
    async.auto({
        checkUserExistsinDB: (cb) => {
            if (!data.amount) {
                cb(null, { "statusCode": util.statusCode.BAD_REQUEST, "statusMessage": util.statusMessage.PARAMS_MISSING })
            ...
            let criteria = {
                ...
                payment_type: "Credit",
                paid_deposit_by: "Online",
                description: "Recharge From Paytm"
            }
            customerDAO.addWalletAmount(criteria, (err, dbData) => {
                if (err) {
                    cb(null, { "statusCode": util.statusCode.FOUR_ZERO_FOUR, "statusMessage": util.statusMessage.DB_ERROR });
                    return;
                }
                else {
                    customerDAO.addWalletHistory(criteria, (err, dbData1) => {
                        if (err) {
                            cb(null, { "statusCode": util.statusCode.FOUR_ZERO_FOUR, "statusMessage": util.statusMessage.DB_ERROR });
                            return;
                        } else {
                            cb(null, { "statusCode": util.statusCode.OK, "statusMessage": "Wallet Recharge Successfully"});
                            return;
                        }
                    });
                }
...
//=================== order detail ======================//
let getWalletHistory = (data, headers, cb) => {
    async.auto({
            checkUserExistsinDB: (cb) => {
                var userId
                util.jwtDecode(headers.accesstoken, (err, token) => {
                ...
                customerDAO.getUserData(criteria, (err, dbData) => {
                    ...
                    if (dbData && dbData.length == 0) {
                        ...
                        customerDAO.getWalletHistory(criteria, (err1, dbData1) => {
                            if (err1) {
                                cb(null, { "statusCode": util.statusCode.FOUR_ZERO_FOUR, "statusMessage": util.statusMessage.DB_ERROR });
                                return;
                            } else {
                                cb(null, { "statusCode": util.statusCode.OK, "statusMessage": "Wallet History", "user_data": dbData, "wallet_history": dbData1 });
                                return;
                               }
                        });
                    }

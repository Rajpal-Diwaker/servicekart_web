...
let getCategory = (criteria, callback) => {
    ...
    dbConfig.getDB().query(`select DISTINCT a.id,a.name as category_name,a.description,a.category_image as image,a.status,a.added_on,round(( 6371 * acos( cos( radians('${criteria.lat}') ) * cos( radians( v.lat ) ) * cos( radians( v.lng ) - radians('${criteria.lng}') ) + sin( radians('${criteria.lat}') ) * sin( radians( v.lat ) ) ) )) AS distance from s_users as u left join s_vendors as v on u.user_id = v.user_id left join s_inventory as n on u.user_id = n.vendor_id left join s_categories as a on n.category_id = a.id where ${conditions} HAVING distance < 10 
    ORDER BY distance`, async function (err, res) {
...
let getDailyServices = (criteria, callback) => {
    ...
    dbConfig.getDB().query(`select DISTINCT a.id,a.name as category_name,a.description,a.category_image as image,a.status,a.added_on,round(( 6371 * acos( cos( radians('${criteria.lat}') ) * cos( radians( v.lat ) ) * cos( radians( v.lng ) - radians('${criteria.lng}') ) + sin( radians('${criteria.lat}') ) * sin( radians( v.lat ) ) ) )) AS distance from s_users as u left join s_vendors as v on u.user_id = v.user_id left join s_inventory as n on u.user_id = n.vendor_id left join s_categories as a on n.category_id = a.id where ${conditions} HAVING distance < 10 ORDER BY distance limit 4`, async function (err, res) {
        if (err) {
            callback(err, null)
        }
        else {
                ...
                if (res && res.length > 0) {
                    ...
                    if (add && add.length > 0) {
                        ...
                        let shipCharge = await new Promise((resolve, reject) => {
                            dbConfig.getDB().query(`SELECT a.base_delivery_charge , a.delivery_charge_per_km , a.minimum_order, a.base_delivery_distance, 
                                111.111 *
                                 DEGREES(ACOS(LEAST(1.0, COS(RADIANS(a.lat))
                                      * COS(RADIANS(${add[0].lat}))
                                      * COS(RADIANS(a.lng - ${add[0].lng}))
                                      + SIN(RADIANS(a.lat))
                                      * SIN(RADIANS(${add[0].lat}))))) AS distance_in_km
                               FROM s_vendors AS a
                   ...
                   console.log('coupan_detail--------------------->>>>>>',coupan_detail)
                    ...
                    dbConfig.getDB().query(`SELECT c.*,s.quantity as update_quantity,IFNULL(ROUND(v.unit_price,2),ROUND(i.base_price,2)) as price FROM s_cart_items as c left join s_inventory_variants as v on v.id = c.variant_id left join s_inventory as i on i.id = c.item_id left join s_subscriptions as s on s.item_id = c.item_id WHERE c.cart_id = '${cart_id}' and c.product_order_type = '${tab}'`, function (err1, res1) {
                        ...
                        if (err1) {
                            ...
                            if (res1.length > 0) {
                                ...
                                dbConfig.getDB().query("insert into s_orders set ? ", data1, function (err2, orderId) {
                                    console.log('model_cart_error222++++++++++>', err2)
                                    console.log(err2);
                                    let order_latest_id = orderId.insertId
                                    for (const [key, value] of Object.entries(res1)) {
                                        var order_detail = {
